...
export async function GET(request: NextRequest) {
  try {
    ...
    // Use the 'page_size' parameter to limit the number of results (e.g., only recent tasks)
    const response = await fetch(`https://api.twelvelabs.io/v1.3/tasks?index_id=${indexId}&page_size=10&page=1`, {
      method: 'GET',
      headers: {
        'x-api-key': apiKey,
